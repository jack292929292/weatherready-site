...
def send_email(to_email, subject, forecast_text, transaction_id, amount_paid):
    ...
    html_details = "".join(f"<li>{line}</li>" for line in lines)
    html_content = f"""
    <html>
...
def create_checkout_session():
    ...
    selected_dates = data.get("forecast_dates", [])
    email = data.get("email", "")
    pricing = [0.99, 1.87, 2.65, 3.35, 3.96, 4.51, 5.00]
    price = int(pricing[min(len(selected_dates), 7) - 1] * 100)
    session = stripe.checkout.Session.create(
        ...
        line_items=[{
            "price_data": {
                "currency": "aud",
                "product_data": {
                    "name": f"{len(selected_dates)} Forecast(s): {', '.join(selected_dates)}",
                },
                "unit_amount": price,
            },
            "quantity": 1,
        ...
        success_url=url_for("success", _external=True) +
                    f"?dates={'|'.join(selected_dates)}&email={email}&amount={price}&session_id={{CHECKOUT_SESSION_ID}}",
...
def success():
    ...
    amount_paid = int(request.args.get("amount", 0)) / 100
    ...
    try:
        ...
        for date in dates:
            try:
                ...
                forecast_text = f"{date}: Max Temp: {max_temp:.1f}°C\nRainfall: {rain_prob:.0f}% chance of {rain_amt:.1f} mm"
                all_forecasts.append(forecast_text)
            except Exception as e:
                all_forecasts.append(f"{date}: Forecast not available. DATA ERROR: {e}")
        combined_forecast = "\n\n".join(all_forecasts)
        ...
        send_email(
            to_email=email,
            subject=f"Your Long-Range Weather Forecast – {', '.join(dates)}",
            forecast_text=combined_forecast,
            transaction_id=full_order_id,
            amount_paid=amount_paid
        )
        for i, date in enumerate(dates):
            log_order_to_sheets(order_id, stripe_id, email, date, all_forecasts[i])
    except Exception as e:
        combined_forecast = f"Error generating forecasts or sending email. ERROR: {e}"
    return render_template("result.html", forecast=combined_forecast, date=", ".join(dates))
